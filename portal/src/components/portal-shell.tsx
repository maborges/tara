...
const SCOPES = [
  ...
  ["operators:read", "Consultar operadores", "Permite consultar os operadores disponíveis para a operação."],
  ["operators:write", "Administrar operadores", "Permite cadastrar e revogar operadores da Conta."],
...
function Dashboard({ session, onSignOut }: { session: Session; onSignOut: () => void }) {
  ...
  return (
    <SidebarProvider>
          ...
          <div className="flex-1 p-6 overflow-auto">
            ...
            {view === "api-keys" && (
              <div className="space-y-6">
                ...
                {loading ? <Card><CardContent className="space-y-2 p-5"><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-full" /></CardContent></Card> : clients.length ? <ClientTable clients={clients} session={session} onRefresh={() => void load()} onCredential={setCredential} onConfiguration={setConfigurationClient} onEdit={setEditingClient} onError={setError} /> : <Card><EmptyState onCreate={() => setShowForm(true)} /></Card>}
              </div>
            )}
      ...
      {editingClient && <EditKeyDialog client={editingClient} session={session} onClose={() => setEditingClient(null)} onSaved={() => { setEditingClient(null); void load(); }} onError={setError} />}
    </SidebarProvider>
  );
}
...
function ClientActions({ client, session, onRefresh, onCredential, onConfiguration, onEdit, onError }: { client: ApiClient; session: Session; onRefresh: () => void; onCredential: (value: NewCredential) => void; onConfiguration: (client: ApiClient) => void; onEdit: (client: ApiClient) => void; onError: (value: string) => void }) {
  ...
  return <div className="flex justify-end gap-1"><Button variant="ghost" size="icon-sm" disabled={busy} title="Editar permissões" onClick={() => onEdit(client)}><Pencil /></Button><Button variant="ghost" size="icon-sm" disabled={busy} title="Ver configuração da aplicação cliente" onClick={() => onConfiguration(client)}><Settings2 /></Button><Button variant="ghost" size="icon-sm" disabled={busy} title="Resetar/rotacionar segredo" onClick={() => void rotate()}><RotateCcw /></Button>{client.status === "ATIVO" && <Button variant="ghost" size="icon-sm" disabled={busy} title="Revogar" onClick={() => void revoke()}><span className="text-destructive">×</span></Button>}</div>;
}

function EditKeyDialog({ client, session, onClose, onSaved, onError }: { client: ApiClient; session: Session; onClose: () => void; onSaved: () => void; onError: (value: string) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try {
      await updateApiClient(session, client.client_id, { nome: name.trim(), scopes, expires_at: client.expires_at });
      toast.success("Permissões da API Key atualizadas.");
      onSaved();
    } catch (cause) {
      onError(cause instanceof Error ? cause.message : "Falha ao atualizar a API Key.");
    } finally { setBusy(false); }
  }
  return <Dialog title={`Editar API Key — ${client.nome}`} onClose={onClose}><form className="mt-5 space-y-4" onSubmit={submit}><Field label="Nome da integração"><Input required value={name} onChange={(event) => setName(event.target.value)} /></Field><div className="space-y-2"><strong className="text-sm font-medium">Permissões</strong><p className="text-xs text-muted-foreground">Altere somente o necessário. O Client Secret permanece o mesmo.</p>{SCOPES.map(([scope, label, help]) => <label className="flex items-center gap-2 text-sm" key={scope}><input type="checkbox" checked={scopes.includes(scope)} onChange={(event) => setScopes((current) => event.target.checked ? [...current, scope] : current.filter((item) => item !== scope))} /><span className="flex-1">{label}</span><span title={help} aria-label={`Ajuda: ${help}`} role="img" className="inline-flex cursor-help text-muted-foreground hover:text-primary"><HelpCircle className="size-4" /></span></label>)}</div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={busy || !name.trim() || !scopes.length}>{busy ? "Salvando…" : "Salvar alterações"}</Button></div></form></Dialog>;
}
