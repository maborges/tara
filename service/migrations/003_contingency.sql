...
alter table tara.estacoes add column if not exists public_key jsonb;
alter table tara.estacoes add column if not exists identity_fingerprint varchar(64);

create table if not exists tara.contingencia_lotes (
    id uuid primary key,
    tenant_id uuid not null,
    station_id uuid not null references tara.estacoes(id),
    ...
    constraint uq_TARA_contingencia_package unique (tenant_id, package_id),
    constraint uq_TARA_contingencia_sequence unique (tenant_id, station_id, sequence_number)
);

create table if not exists tara.contingencia_itens (
    id uuid primary key,
    tenant_id uuid not null,
    lote_id uuid not null references tara.contingencia_lotes(id) on delete cascade,
    local_id varchar(120) not null,
    status varchar(20) not null,
    pesagem_id uuid references tara.pesagens(id),
    error_message text,
    payload jsonb not null,
    created_at timestamp not null,
    constraint uq_TARA_contingencia_local unique (tenant_id, local_id)
);
