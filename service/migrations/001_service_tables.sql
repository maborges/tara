...
-- Executar depois da fundação balanca.contas/clientes/eventos_outbox.

create schema if not exists balanca;

create table if not exists balanca.estacoes (
    id uuid primary key,
    ...
    constraint uq_balanca_estacoes_external unique (tenant_id, external_id)
);

create table if not exists balanca.operadores (
    id uuid primary key,
    tenant_id uuid not null,
    ...
    status varchar(20) not null,
    created_at timestamp not null,
    constraint uq_balanca_operadores_codigo unique (tenant_id, codigo)
);

create table if not exists balanca.ordens (
    id uuid primary key,
    tenant_id uuid not null,
    cliente_id uuid not null references balanca.clientes(id),
    ...
    constraint uq_balanca_ordens_external unique (tenant_id, sistema_cliente, referencia_externa)
);

create index if not exists ix_balanca_ordens_status
    on balanca.ordens(tenant_id, status);

create table if not exists balanca.pesagens (
    id uuid primary key,
    tenant_id uuid not null,
    ordem_id uuid not null references balanca.ordens(id) on delete cascade,
    ...
    operador_id uuid references balanca.operadores(id),
    leitura_bruta jsonb,
    captured_at timestamp not null,
    constraint uq_balanca_pesagens_local unique (tenant_id, local_id),
    constraint uq_balanca_pesagens_ordem_etapa unique (ordem_id, etapa)
);
