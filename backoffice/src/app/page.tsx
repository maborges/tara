...
export default function BackofficePage() {
  const [session, setSession] = useState<Session | null>(null); const [view, setView] = useState<View>("overview"); const [dark, setDark] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null);
        ...
        <Sidebar collapsible="icon">
          <SidebarHeader>
            <div className="flex items-center gap-2 p-2">
              <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground font-bold">T</div>
              <div className="flex flex-col truncate group-data-[collapsible=icon]:hidden">
                <strong className="text-sm">Tara</strong>
                <span className="text-xs text-muted-foreground">Plataforma operacional</span>
              </div>
            </div>
          </SidebarHeader>
          <SidebarContent>
                ...
                <SidebarMenu>
                  {nav.map(([key, Icon, label, permission]) => permission && !can(permission) ? null : (
                    <SidebarMenuItem key={key}>
                      <SidebarMenuButton isActive={view === key} onClick={() => setView(key)} tooltip={label}>
...
function LoginScreen({ onLogin }: { onLogin: (session: Session) => void }) { 
  ...
  return (
    <div className="flex min-h-screen bg-muted/30">
      <div className="flex-1 flex flex-col justify-center items-center bg-primary text-primary-foreground p-12">
        <div className="max-w-md space-y-6">
          <div className="flex items-center gap-4">
            <div className="flex size-12 items-center justify-center rounded-xl bg-background text-primary font-bold text-2xl">T</div>
            <div><h1 className="text-2xl font-bold tracking-tight">Tara</h1><p className="text-primary-foreground/80">Plataforma operacional</p></div>
          </div>
          <div><h2 className="text-4xl font-bold tracking-tight">Controle claro para cada pesagem.</h2><p className="mt-4 text-lg text-primary-foreground/80">Administre clientes, credenciais, estações, operadores e eventos em um único lugar.</p></div>
        </div>
      </div>
...
function ClientPanel({ clients, session, onRefresh, onError }: { clients: ApiClient[]; session: Session; onRefresh: () => void; onError: (value: string | null) => void }) { 
  ...
  async function rotate(clientId: string) { if (!window.confirm("Rotacionar a credencial? O segredo atual deixará de funcionar.")) return; try { setCredential(await rotateApiClient(session, clientId)); onRefresh(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Falha ao rotacionar credencial."); } } 
  async function revoke(clientId: string) { if (!window.confirm("Revogar esta credencial?")) return; try { await revokeApiClient(session, clientId); onRefresh(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Falha ao revogar credencial."); } } 
  ...
  const columns: ColumnDef<ApiClient>[] = [
    ...
    { id: "acoes", header: "Ações", cell: (val, row) => (
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon-sm" onClick={() => void rotate(row.client_id)} title="Rotacionar"><RotateCcw size={14} /></Button>
...
function OperatorPanel({ operators, session, onRefresh, onError }: { operators: Operator[]; session: Session; onRefresh: () => void; onError: (value: string | null) => void }) { 
  ...
  async function revoke(id: string) { if (!window.confirm("Revogar este operador?")) return; try { await revokeOperator(session, id); onRefresh(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Falha ao revogar operador."); } } 
